...
-- ============================================================
-- REMOVE CheckPlayerSpeed (impede detecção de speed hack)
-- ============================================================
...
-- ============================================================
-- SPRINT EVENT HOOK
-- Intercepta SprintEvent para WalkSpeed/RunSpeed custom
-- ============================================================
local sprintActive = false
-- Detecta velocidade atual do Humanoid como valor padrão
local function getBaseSpeed()
    local ch = LP.Character or LP.CharacterAdded:Wait()
    local hum = ch and ch:FindFirstChildOfClass("Humanoid")
    return hum and hum.WalkSpeed or 16
end
local walkSpeedVal = getBaseSpeed()
local runSpeedVal  = walkSpeedVal * 2  -- RunSpeed = 2x o walk detectado
...
-- Hook no FireServer do SprintEvent
pcall(function()
    local events = RS:WaitForChild("Events", 3)
...
-- AUTO UPDATE
-- ============================================================
local UPDATE_URL = "https://raw.githubusercontent.com/coolkidd488/Aurora-Hub/main/AuroraHub.lua"

local function checkUpdate()
    local ok, result = pcall(function()
        return game:GetService("HttpService"):GetAsync(UPDATE_URL, true)
    end)
    if not ok or not result then return end

    -- Extrai versão do script remoto
    local remoteVersion = result:match('local VERSION = "([%d%.]+)"')
    if not remoteVersion then return end

    -- Compara versões (ex: "0.2.7" vs "0.3.0")
    local function parseVer(v)
        local a,b,c = v:match("(%d+)%.(%d+)%.(%d+)")
        return tonumber(a)*10000 + tonumber(b)*100 + tonumber(c)
    end

    if parseVer(remoteVersion) > parseVer(VERSION) then
        -- Mostra notificação no hub
        local notif = Instance.new("Frame")
        notif.Size = UDim2.new(1,0,0,48)
        notif.Position = UDim2.new(0,0,0,0)
        notif.BackgroundColor3 = Color3.fromRGB(0,160,60)
        ...
        notifLbl.Size = UDim2.new(1,-110,1,0)
        notifLbl.Position = UDim2.new(0,10,0,0)
        notifLbl.BackgroundTransparency = 1
        notifLbl.Text = "✦ Nova versão v"..remoteVersion.." disponível!"
        ...
        updateBtn.Size = UDim2.new(0,90,0,32)
        updateBtn.Position = UDim2.new(1,-98,0.5,-16)
        updateBtn.BackgroundColor3 = Color3.fromRGB(255,255,255)
        updateBtn.BackgroundTransparency = 0.1
        updateBtn.Text = "Atualizar"
        updateBtn.TextColor3 = Color3.fromRGB(0,130,50)
        updateBtn.TextSize = 12
        updateBtn.Font = Enum.Font.GothamBold
        updateBtn.BorderSizePixel = 0
        updateBtn.ZIndex = 52
        updateBtn.Parent = notif
        mkCorner(updateBtn, 6)

        updateBtn.MouseButton1Click:Connect(function()
            -- Executa o script novo
            local ok2, err = pcall(loadstring, result)
            if ok2 then
                notif:Destroy()
                loadstring(result)()
            else
                notifLbl.Text = "❌ Erro ao atualizar: "..tostring(err)
            end
        end)

        -- Auto-esconde após 30s se não interagir
        task.delay(30, function()
            if notif and notif.Parent then
                tw(notif, TF, {BackgroundTransparency=1})
...
-- Checa atualização 3 segundos após carregar
task.delay(3, function() pcall(checkUpdate) end)
