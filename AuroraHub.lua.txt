--[[  AURORA HUB v0.2.9.8 unix  ]]

local VERSION = "0.2.9.8 unix fix?"
...
local baseSpeed = walkSpeedVal -- velocidade original do jogo
...
-- ============================================================
-- SAFE NOCLIP
-- Loop no RenderStepped — desativa CanCollide de partes
-- laterais próximas, protege chão e teto
-- ============================================================
local noclipEnabled = false
local noclipParts = {} -- partes com CanCollide desativado

local RS2 = game:GetService("RunService")

RS2.Heartbeat:Connect(function()
    if not noclipEnabled then
        -- Restaurar tudo ao desativar
    ...
    for part in pairs(noclipParts) do
        if not part or not part.Parent then
            ...
            if (part.Position - hrpPos).Magnitude > 6 then
                pcall(function() part.CanCollide = true end)
                noclipParts[part] = nil
            end
        end
    ...
    -- Desativar partes próximas laterais
    for _, part in ipairs(workspace:GetDescendants()) do
        if part:IsA("BasePart") and part.CanCollide and not noclipParts[part] then
            if not ch:IsAncestorOf(part) then
                local dist = (part.Position - hrpPos).Magnitude
                if dist <= 5 then
                    local diffY = part.Position.Y - hrpY
                    -- Protege chão (abaixo) e teto (acima)
                    local isFloor   = diffY < -1.2
                    local isCeiling = diffY > 2.2
                    if not isFloor and not isCeiling then
...
do
    ...
    o=o+2 toggleRow(pg,"Ativar Walk Speed",walkEnabled,o,function(v)
    ...
    ncDesc.Text="Desativa colisao de paredes proximas. Chao/teto protegidos."
    ...
    o=o+2 toggleRow(pg,"Ativar Safe Noclip",noclipEnabled,o,function(v) noclipEnabled=v end)
    o=o+1 divider(pg,o)
end
