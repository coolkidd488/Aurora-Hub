...
local function setupMachine(obj)
    ...
    task.spawn(function()
        while obj.Parent do
            -- reaplicar estado continuamente para refletir mudanças da UI
            apply(cur)
...
local BMin   = winBtn("─",258,BLUE)
local BClose = winBtn("✕",292,RED)
...
BMin.MouseButton1Click:Connect(function()
    if isMin then
        ...
        tw(WIN,TF,{Size=UDim2.new(0,300,0,48)})
    end
end)

BClose.MouseButton1Click:Connect(function()
    isOpen=false isMin=false
    tw(WIN,TF,{Size=UDim2.new(0,300,0,0)})
