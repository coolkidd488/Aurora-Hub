...
-- Executa flip: dir=1 para frente, dir=-1 para trás
local function doFlip(direction)
    ...
    local RS_flip = game:GetService("RunService")

    -- Salva estado
    local origGrav = workspace.Gravity
    local origJump = hum.JumpHeight or hum.JumpPower

    -- Impulso inicial leve para dar altura ao flip
    local bv = Instance.new("BodyVelocity")
    bv.Velocity = Vector3.new(
        hrp.CFrame.LookVector.X * 12 * direction,
        28,
        hrp.CFrame.LookVector.Z * 12 * direction
    )
    bv.MaxForce = Vector3.new(1e4, 1e4, 1e4)
    bv.P = 1e4
    bv.Parent = hrp

    -- Rotaciona 360° em ~0.55s (18 frames de ~0.03s cada)
    local STEPS    = 18
    local ROT_STEP = (math.pi * 2) / STEPS * direction  -- radianos por step

    task.spawn(function()
        -- Remove BodyVelocity após 0.18s (impulso aplicado)
        task.wait(0.18)
        ...
        local step = 0
        local conn
        conn = RS_flip.Heartbeat:Connect(function()
            step = step + 1
            pcall(function()
                -- Rotaciona em torno do eixo X local (mortal p/ frente/trás)
                hrp.CFrame = hrp.CFrame * CFrame.Angles(ROT_STEP, 0, 0)
            end)
            if step >= STEPS then
                conn:Disconnect()
                -- Normaliza CFrame para evitar drift de rotação
                pcall(function()
                    local p = hrp.Position
                    local look = hrp.CFrame.LookVector
                    hrp.CFrame = CFrame.new(p, p + Vector3.new(look.X, 0, look.Z))
                end)
            end
        end)
    end)
...
local function createFlipTool(name, direction)
    ...
    tool.ToolTip = name .. " — clique para executar"

    -- Ícone visual (label no tool)
    local lbl = Instance.new("TextLabel", tool)
    lbl.Name = "Handle"  -- evita erro de handle missing em alguns jogos
    lbl.Size = UDim2.new(1,0,1,0)
    lbl.BackgroundTransparency = 1
    lbl.Text = direction == 1 and "🔄" or "🔃"
    lbl.TextSize = 24
    ...
    tool.Activated:Connect(function()
        ...
        debounce = true
        doFlip(direction)
        task.wait(1.0)  -- cooldown entre flips
        debounce = false
    end)
...
local function enableFrontFlip()
    if _frontFlipTool then _frontFlipTool:Destroy() end
    _frontFlipTool = createFlipTool("Front Flip", 1)
    -- Coloca na Backpack do jogador local
    local bp = LP:FindFirstChildOfClass("Backpack")
    if bp then _frontFlipTool.Parent = bp end
end

local function disableFrontFlip()
    if _frontFlipTool then
        _frontFlipTool:Destroy()
        _frontFlipTool = nil
    end
end

local function enableBackFlip()
    if _backFlipTool then _backFlipTool:Destroy() end
    _backFlipTool = createFlipTool("Back Flip", -1)
    local bp = LP:FindFirstChildOfClass("Backpack")
    if bp then _backFlipTool.Parent = bp end
end

local function disableBackFlip()
    if _backFlipTool then
        _backFlipTool:Destroy()
        _backFlipTool = nil
    end
end
...
LP.CharacterAdded:Connect(function(newChar)
    ...
    if _frontFlipTool then
        _frontFlipTool:Destroy()
        _frontFlipTool = createFlipTool("Front Flip", 1)
        _frontFlipTool.Parent = bp
    end
    if _backFlipTool then
        _backFlipTool:Destroy()
        _backFlipTool = createFlipTool("Back Flip", -1)
        _backFlipTool.Parent = bp
    end
end)
